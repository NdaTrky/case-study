<template>
  <section class="container mx-auto px-4 sm:px-5 py-8 md:py-12">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mb-6 md:mb-8">
      ...
      <a href="#" class="text-gray-500 text-xs sm:text-sm flex items-center mt-3 sm:mt-0">
        All Deals
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:gap-y-8 gap-6 md:gap-8 place-items-center">
      <div v-for="deal in dealsOfTheDay" :key="deal.id" class="relative group w-full max-w-xs space-y-4">
        ...
        <div class="absolute w-[calc(100%-16px)] sm:w-[calc(100%-20px)] left-1/2 -translate-x-1/2 bottom-0 transform translate-y-1/2 bg-white rounded-md sm:rounded-lg shadow-xs sm:shadow-sm p-2 sm:p-3 md:p-4 border border-gray-100">
          <p class="text-primary-gray text-[10px] xs:text-xs mb-1 truncate">{{ deal.brand }}</p>
          ...
          <div class="flex items-center justify-between">
            <!-- Fiyatlar -->
            <div>
              <span class="text-primary-green font-bold text-xs sm:text-sm">${{ deal.price.toFixed(2) }}</span>
              <span class="text-primary-gray line-through text-[10px] xs:text-xs ml-1 sm:ml-2">${{ deal.oldPrice.toFixed(2) }}</span>
            </div>
